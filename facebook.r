...
dir.create( "/Users/Chen/Documents/facebookFriends/photos" )
photos <- facebook( path="me/photos",access_token=myaccess_token )
sapply( photos$data, function(x){
    url <- x$source
    download.file( url, file.path( "/Users/Chen/Documents/facebookFriends/photos", basename(url) ) )
})
...
pdf(file="/Users/Chen/Documents/facebookFriends/facebook1.pdf", width=25, height=25)
...
dir.create("/Users/Chen/Documents/facebookFriends/photos2")

for (i in 1:length(friends.id))
download.file(paste("http://graph.facebook.com", friends.id[i], "picture", sep="/"),destfile=paste("/Users/Chen/Documents/facebookFriends/photos2/",friends.id[i],".jpg",sep=""))
 

system('for i in `ls /Users/Chen/Documents/facebookFriends/photos2/*.jpg`; do j=${i%.*}; convert $j.jpg $j.jpg; done', wait=TRUE)


V(g)$label=""

l<-L
pdf(file="/Users/Chen/Documents/facebookFriends/facebook2.pdf", width=25, height=25)
plot(g,layout=L,vertex.size=0,vertex.frame.color="#00000000")
for(i in length(friends.id):1){			  
 	myfriend.photo<-read.jpeg(paste("/Users/Chen/Documents/facebookFriends/photos2/",friends.id[i],".jpg",sep=""))
	if(i!=1){
